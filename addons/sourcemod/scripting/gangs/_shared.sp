stock void PushClientArray(int client)
{
	...
	Format(sQuery, sizeof(sQuery), "SELECT GangID, CommunityID, PlayerName, AccessLevel, Muted FROM `jailbreak_gangs_members` WHERE `CommunityID` = '%s'", g_sClientID[client]);
...
stock void UpdateClientMuteState(const char[] communityid, bool status)
{
	for (int i = 0; i < g_aCacheGangMembers.Length; i++)
	{
		...
		if (StrEqual(iGang[sCommunityID], communityid))
		{
			...
			if(status)
				CPrintToGang(iGang[iGangID], "{darkred}[%s] \x07%s \x06został wyciszony!", g_sGang[iGang[iGangID]], iGang[sPlayerN]);
			else
				CPrintToGang(iGang[iGangID], "{darkred}[%s] \x07%s nie \x06jest już wyciszony!", g_sGang[iGang[iGangID]], iGang[sPlayerN]);
