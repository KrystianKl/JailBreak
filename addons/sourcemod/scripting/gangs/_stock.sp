...
stock int AddGangPoints(int gangid, int points)
{
	for (int i = 0; i < g_aCacheGang.Length; i++)
	{
		...
		if (iGang[iGangID] == gangid)
		{
			...
			char sQuery[512];
			Format(sQuery, sizeof(sQuery), "UPDATE `jailbreak_gangs` SET `Points`= '%d' WHERE `GangID` = '%d'", tmp_iPoints, gangid);
			SQLQuery(sQuery);
...
stock int AddGangCredits(int gangid, int credits)
{
	for (int i = 0; i < g_aCacheGang.Length; i++)
	{
		int iGang[Cache_Gang];
		...
		if (iGang[iGangID] == gangid)
		{
			int tmp_iCredits = iGang[iCredits] + credits;
			g_aCacheGang.Set(i, tmp_iCredits, view_as<int>(iCredits));
			
			char sQuery[512];
			Format(sQuery, sizeof(sQuery), "UPDATE `jailbreak_gangs` SET `Credits`= '%d' WHERE `GangID` = '%d'", tmp_iCredits, gangid);
			SQLQuery(sQuery);
...
stock int RemoveGangCredits(int gangid, int credits)
{
	for (int i = 0; i < g_aCacheGang.Length; i++)
	{
		int iGang[Cache_Gang];
	...
	for (int i = 0; i < g_aCacheGangMembers.Length; i++)
	{
		...
		if(StrEqual(iGangMembers[sCommunityID], g_sClientID[client]))
		{
			if(!StrEqual(iGangMembers[sPlayerN], newname, false))
			{
				...
				Format(sQuery, sizeof(sQuery), "UPDATE `jailbreak_gangs_members` SET `PlayerName` = '%s' WHERE `CommunityID` = '%s'", sEName, g_sClientID[client]);
				Format(sQuery, sizeof(sQuery), "UPDATE `jailbreak_gangs_trans` SET `PlayerName` = '%s' WHERE `CommunityID` = '%s'", sEName, g_sClientID[client]);
				SQLQuery(sQuery);
			}
		}
	}
