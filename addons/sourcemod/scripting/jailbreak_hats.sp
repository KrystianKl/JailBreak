...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_editorhats", DOMenu, ADMFLAG_ROOT, "Opens hats editor.");
...
{	
	LoadHats();
	CPrintToChat(client, "\x0B[EverGames]\x06 Konfiguracja została przeładowana!");
	return Plugin_Handled;
}

void Showmenuh(int client, int item2)
{
	SetMenuTitle(menu_hats, "EverGames.pl » Wybierz Czapkę", client);
...
public int DIDMenuHandler(Menu menu, MenuAction action,int client,int itemNum) 
{
	if ( action == MenuAction_Select ) 
	{
		...
		{
			CPrintToChat(client, "\x02[EverGames] \x07Nie masz dostępu do tej komendy!");
			Showmenuh(client, GetMenuSelectionPosition());
			return;
		}
		RemoveHat(client);
		g_Elegido[client] = index;
		CPrintToChat(client, "\x0B[EverGames]\x06 Wybrałeś: \x03%s\x06!", g_eHats[g_Elegido[client]][Name]);
		CreateHat(client);
		Showmenuh(client, GetMenuSelectionPosition());
	}
...
public void LoadHats()
{
	...
	g_hats = 0;
	BuildPath(Path_SM, sConfig, PLATFORM_MAX_PATH, "configs/jailbreak_hats.txt");
	...
	SetMenuTitle(menu_hats, "EverGames.pl » Wybierz Czapkę");
	char item[4];
	for (int i=0; i<g_hats; ++i) {
		Format(item, 4, "%i", i);
	...
	SetMenuTitle(menu_editor, "EverGames.pl » Edytor Czapek");
...
public Action DOMenu(int client,int args)
{
	...
	{
		CPrintToChat(client, "\x02[EverGames] \x07Najpierw wybierz czapkę!");
		return Plugin_Handled;
	}
	
	Menu menu_editor_init = new Menu(DIDMenuHandler_init);
	SetMenuTitle(menu_editor_init, "EverGames.pl » Edytor Czapek");
	
	AddMenuItem(menu_editor_init, "default", "Edytuj domyślną pozycję czapki");
	AddMenuItem(menu_editor_init, "model", "Edytuj pozycję czapki dla modelu");
...
{
	SetMenuTitle(menu_editor2, "EverGames.pl » Edytor Czapek");
...
public int DIDMenuHandler2(Menu menu, MenuAction action, int client, int itemNum) 
{
	if ( action == MenuAction_Select ) 
	{
		...
		else if (StrContains(info, "Save", false) != -1)
		{
			...
			SetEntProp(client, Prop_Send, "m_iHideHUD", GetEntProp(client, Prop_Send, "m_iHideHUD") | HIDE_RADAR_CSGO);
			CPrintToChat(client, "\x0B[EverGames]\x06 Konfiguracja zapisana!");
		}
